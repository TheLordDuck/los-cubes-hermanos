  ...
  return (
    ...
    >
      <div className="relative w-full aspect-video bg-neutral-100 dark:bg-neutral-800 overflow-hidden">
        ...
        <span className={`absolute top-2 left-2 text-[10px] font-medium px-1.5 py-0.5 rounded uppercase tracking-wide ${
          cube.type === 'BATTLEBOX' ? 'bg-blue-950 text-blue-300' : 'bg-emerald-950 text-emerald-300'
        }`}>
          {cube.type === 'BATTLEBOX' ? 'Battlebox' : 'Cube'}
        </span>

        {cube.isTwobert && (
          <span className="absolute top-2 left-14 text-[10px] font-medium px-1.5 py-0.5 rounded uppercase tracking-wide bg-purple-950 text-purple-300">
            Twobert
          </span>
        )}

        <span className={`absolute top-2 right-2 text-[10px] font-medium px-1.5 py-0.5 rounded uppercase tracking-wide ${
          DIFFICULTY_STYLES[cube.difficulty.toLowerCase()] ?? 'bg-neutral-800 text-neutral-300'
        }`}>
          {cube.difficulty}
        </span>
      </div>
